...
{
    int nrRooms=8;
    std::string inputUsernameText = "Hejsan";
    std::string inputPasswordText = "";
    //mainUI(nrRooms,inputUsernameText,inputPasswordText);
    //user_s user1;
    //char msgen[20];
    //user1.username = string_convert("Hello", msgen);
    //string_convert(inputUsernameText, user1.username);
    if (!initGui()) {
        printf( "Failed to during gui initialization!\n" );
    }else{
        /*printf("Hello");
        while (1) {
            //printf("Hello\n");
...
void string_convert(std::string s,char* msgen[20]){
    int i = 0, j = s.size();
    for(i;i <= j;i++){
        msgen[i] = & s[i];
    }
