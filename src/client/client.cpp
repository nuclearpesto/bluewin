   ...
   Readstruct *r = (Readstruct *) p;
    ...
    while(numBytesRead>-1){
        ...
        if(masterobj == NULL){
           ...
           else if((keycheckobj = json_object_get(masterobj, "get users in room")) != NULL){//Reads how many users there is in a room
            ...
            else if((keycheckobj = json_object_get(masterobj, "add user")) != NULL){ //adds a new user
			if(json_is_true(keycheckobj)){
					*createCheck = true;
				}
			}
...
void message_printer(json_t *masterobj){//Prints out the message and the name of the user who wrote it
...
void add_room(json_t *masterobj, char *room, TCPsocket *socket, SDL_mutex *writeMutex){//add/create a new room
    serialize_room(masterobj, room);
...
void add_user(json_t * masterobj, user_s *usr, TCPsocket *sd, SDL_mutex *writeMutex){//add a new user
    serialize_username(masterobj,usr);
    serialize_password(masterobj,usr);
...
void write_message(json_t *masterobj, user_s *usr, Message_s msg, TCPsocket *socket, SDL_mutex *writeMutex){//sends a message to the server/room
	serialize_username(masterobj, usr);
